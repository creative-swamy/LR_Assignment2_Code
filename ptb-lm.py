...
parser.add_argument('--saved_model', type=str, default='',
                    help='path of the saved model to be used instead')
...
if args.saved_model != '':
    model.load_state_dict(torch.load(args.saved_model, map_location='gpu'))
    print('Successfully loaded trained model %s' % args.saved_model)
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        # print("Vocab  shape = ",model.vocab_size)
        ...
        # print("output, targets = ",outputs[0].shape,targets[0].shape)

        if not is_train:
            for i in range(model.seq_len):
                L_t[i] += loss_fn(outputs[i].contiguous().view(-1, model.vocab_size), targets[i])
            print("Step : ",step,"L_t = ",L_t)
            count += 1
        ...
        if not is_train:
            ...
            print("Size = ",len(data) // model.batch_size)
            print("L_t = ",L_t/count)
