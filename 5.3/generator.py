...
parser.add_argument('--hidden_size', type=int, default=2500,
...
def generate_samples(model, generate_length):
    model.eval()    
    ...
    with torch.no_grad():
        ...
        samples = model.generate(0, hidden, generate_length)
    
    for seq_len in range(samples.shape[1]):
        print('sample '+str(seq_len+1)+': ', end='')
        for batch_size in range(samples.shape[0]):
            print(id_2_word[samples[batch_size, seq_len].item()], end=' ')
        print('\n')

if args.model == 'RNN':
  state_dict = torch.load(r"best_params_RNN.pt")
elif args.model == 'GRU':
  state_dict = torch.load(r"best_params_GRU.pt")
model.load_state_dict(state_dict)
generate_samples(model, args.seq_len)
